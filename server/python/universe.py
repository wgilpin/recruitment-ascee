...
def get_corporation_name(corporation_id):
    corporation = Corporation.get_by_id(corporation_id)
...
def get_alliance_name(alliance_id):
    alliance = Alliance.get_by_id(alliance_id)
    if alliance is None:
...
def get_type_price(type_id):
    ...
    price_data = TypePrice.get_by_id(type_id)
...
def get_structure_system(structure_id):
    structure = Structure.get_by_id(structure_id)
